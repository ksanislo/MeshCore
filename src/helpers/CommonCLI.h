...
public:
  ...
  // builds with WITH_MQTT_BRIDGE -- the LVGL companion feeds these via its shim
  // from its own NodePrefs, so they are NOT persisted here).
  uint8_t  mqtt_enabled = 0;        // boolean
  ...
  uint8_t  mqtt_tls = 0;            // boolean; insecure-mode TLS
  uint8_t  mqtt_publish_tx = 0;     // publish post-forward TX bytes to <prefix>/tx
  uint8_t  mqtt_publish_rx = 0;     // publish heard RX bytes to <prefix>/rx
