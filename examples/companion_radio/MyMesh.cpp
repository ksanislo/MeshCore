...
void MyMesh::otaFromUrl() {
  auto set = [this](const char* m) {
    strncpy(_ota_status, m, sizeof(_ota_status) - 1);
    _ota_status[sizeof(_ota_status) - 1] = 0;
  };
...
// if name is provided as a build flag, use that as default node name instead
  ...
  // NOTE: defaults for the fork's appended prefs are applied in ONE place --
  // DataStore::applyAppendedPrefsDefaults() -- which loadPrefs() runs on both the
  // file-read path and the fresh-device no-file path. Do not duplicate them here.
...
void MyMesh::handleCmdFrame(size_t len) {
  if (cmd_frame[0] == CMD_DEVICE_QUERY && len >= 2) { // sent when app establishes connection
    ...
    telemetry.reset();
    if (g_batt_pct >= 0) {   // fuel gauge present: report V + %
      telemetry.addVoltage(TELEM_CHANNEL_SELF, g_batt_mv / 1000.0f);
      telemetry.addPercentage(TELEM_CHANNEL_SELF, (uint32_t)g_batt_pct);
    } else {                 // no monitor: upstream's raw board reading
