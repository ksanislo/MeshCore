 ...
 *
 * RM69A10 panel driver vendored from LilyGo T-Display-P4 (LILYGO_L, GPL 3.0).
 * Pruned: hardcoded RGB565 interface pixel format (dropped the Kconfig
 * CONFIG_SCREEN_PIXEL_FORMAT_* switch and the HI8561/camera lane-select paths).
 */
...
// Vendor-specific init sequence for the RM69A10 (568x1232), RGB565.
                                    ...
                                    esp_lcd_panel_handle_t *ret_panel)
{
    ...
    ESP_GOTO_ON_ERROR(esp_lcd_new_panel_dpi(vendor_config->mipi_config.dsi_bus, vendor_config->mipi_config.dpi_config, ret_panel),
                      err, TAG, "create MIPI DPI panel failed");
    ESP_LOGD(TAG, "new MIPI DPI panel @%p", *ret_panel);

    // Save + overwrite the MIPI DPI panel functions
...
static esp_err_t panel_rm69a10_send_init_cmds(rm69a10_panel_t *rm69a10)
{
    ...
    const rm69a10_lcd_init_cmd_t *init_cmds = vendor_specific_init_default;
    uint16_t init_cmds_size = sizeof(vendor_specific_init_default) / sizeof(rm69a10_lcd_init_cmd_t);
...
static esp_err_t panel_rm69a10_del(esp_lcd_panel_t *panel)
{
    ...
    {
        gpio_reset_pin((gpio_num_t)rm69a10->reset_gpio_num);
    }
    rm69a10->del(panel);
...
static esp_err_t panel_rm69a10_reset(esp_lcd_panel_t *panel)
{
    ...
    if (rm69a10->reset_gpio_num >= 0)
    {
        gpio_set_level((gpio_num_t)rm69a10->reset_gpio_num, rm69a10->flags.reset_level);
...
static esp_err_t panel_rm69a10_sleep(esp_lcd_panel_t *panel, bool sleep)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, sleep ? 0x10 : 0x11, NULL, 0), TAG, "esp_lcd_panel_io_tx_param fail");
...
static esp_err_t panel_rm69a10_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, on_off ? 0x29 : 0x28, NULL, 0), TAG, "esp_lcd_panel_io_tx_param fail");
    vTaskDelay(pdMS_TO_TICKS(120));
    return ESP_OK;
}
...
static esp_err_t panel_rm69a10_invert_color(esp_lcd_panel_t *panel, bool invert_color_data)
{
    ...
    ESP_RETURN_ON_FALSE(io, ESP_ERR_INVALID_STATE, TAG, "invalid panel IO");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, invert_color_data ? LCD_CMD_INVON : LCD_CMD_INVOFF, NULL, 0),
                        TAG, "send command failed");
    return ESP_OK;
}
